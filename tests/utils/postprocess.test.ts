...
describe('extractGameStats', () => {
	describe('with scoreboard table format', () => {
		...
		it('should handle DEFEAT result', () => {
			const regionResults = new Map<string, string>([['result', 'DEFEAT']]);
	...
	describe('case insensitivity', () => {
		it('should handle lowercase result text', () => {
			const regionResults = new Map<string, string>([['result', 'victory']]);
	...
	describe('whitespace handling', () => {
		it('should trim leading and trailing whitespace', () => {
			const regionResults = new Map<string, string>([['blue_player1_name', ' PLAYER ']]);
	...
	describe('formatResults', () => {
		it('formats recognition results into output format', () => {
			const allResults: RecognitionResult[] = [
				{ name: 'player1_name', value: 'TestPlayer', confidence: 95 },
				{ name: 'player1_score', value: '1234', confidence: 88 },
			];

			const result = formatResults(allResults);

			expect(result.ocrTextParts).toHaveLength(2);
			expect(result.ocrTextParts[0]).toBe('player1_name (95%): TestPlayer');
			expect(result.ocrTextParts[1]).toBe('player1_score (88%): 1234');
			expect(result.regionResults.get('player1_name')).toBe('TestPlayer');
			expect(result.regionResults.get('player1_score')).toBe('1234');
		});

		it('returns empty arrays for empty input', () => {
			const result = formatResults([]);

			expect(result.ocrTextParts).toHaveLength(0);
			expect(result.regionResults.size).toBe(0);
		});

		it('handles results with zero confidence', () => {
			const allResults: RecognitionResult[] = [
				{ name: 'failed_region', value: '', confidence: 0 },
			];

			const result = formatResults(allResults);

			expect(result.ocrTextParts[0]).toBe('failed_region (0%): ');
			expect(result.regionResults.get('failed_region')).toBe('');
		});
	});
});
