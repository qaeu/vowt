...
describe('EditableGameData', () => {
	...
	it('should not display action buttons when showActions is false', () => {
		...
		expect(screen.queryByText('Extracted Game Data - Review and Edit')).toBeNull();
	...
	it('should update player field when edited', () => {
		...
		expect(onSaveMock).toHaveBeenCalledWith(
			expect.arrayContaining([expect.objectContaining({ name: 'UpdatedPlayer' })]),
			expect.any(Object)
		);
	});
	...
	it('should hide action buttons when there are no modified fields', () => {
		...
		const saveButton = screen.queryByText(/Save to Records/) as HTMLButtonElement;
		const cancelButton = screen.queryByText(/Reset Changes/) as HTMLButtonElement;
