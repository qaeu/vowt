...
describe('ScoreboardOCR', () => {
  it('should render the component', () => {
    ...
    expect(screen.getByText('Overwatch Scoreboard Tracker POC')).toBeDefined();
  });
  ...
  it('should display preprocessed image after processing', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Preprocessed (Grayscale + Contrast)')).toBeDefined();
    }, { timeout: 3000 });
  });
  ...
  it('should display extracted game stats JSON', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Extracted Game Stats (JSON)')).toBeDefined();
    }, { timeout: 3000 });
  });
  ...
  it('should display success message with count', async () => {
    ...
    await waitFor(() => {
      const successText = screen.queryByText(/Successfully parsed .* data fields/);
  ...
  it('should have correct image sources', () => {
    ...
    const originalImage = screen.getByAltText('Original scoreboard') as HTMLImageElement;
